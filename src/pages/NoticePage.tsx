...
import { getMyClasses } from '../api/students';
import {
  getNotices,
  getNoticeDetail,
  type NoticeListItem,
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      ...
      try {
        ...
        setClasses(
          list.map((c: { classId: number; className: string }) => ({
            classId: c.classId,
    ...
    <MainLayout showCalendar={showCalendar}>
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {isLoadingClasses ? (
          ...
          <span className="text-sm text-slate-500">속한 클래스가 없습니다.</span>
        ) : (
          classes.map(c => (
            <button
              key={c.classId}
